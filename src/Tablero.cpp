...
/*    //El siguiente fragmento de código evita cierto viejo bug que aún no se
    ...
    */
    ...
    if(obtenerTipoDePieza(salida) == DAMA && (_turno == 0) ? !desarrolloBlancasCompleto() : !desarrolloNegrasCompleto()){
        if(_turno == 0){
...
bool Tablero::reyPropioEnJaque(int color) {
    ...
    if(color == 0){
        U64 movimientosDamaNegra;
        U64 bitboardDamaNegra = bitboards[7];
        while(bitboardDamaNegra > 0){
            int casillaDamaNegra = operaciones_bit::LSB(bitboardDamaNegra);
            U64 attackMaskTorre = constantes::attackMasksTorre[casillaDamaNegra-1];
            U64 attackMaskAlfil = constantes::attackMasksAlfil[casillaDamaNegra-1];
            U64 occupancyMask = attackMaskTorre & todas_las_piezas();
            U64 key = occupancyMask * constantes::magicsParaTorre[casillaDamaNegra-1] >> (64-12);
            movimientosDamaNegra = movimientosDeTorre[casillaDamaNegra-1][key];
            occupancyMask = attackMaskAlfil & todas_las_piezas();
            key = occupancyMask * constantes::magicsParaAlfil[casillaDamaNegra-1] >> (64-9);
            movimientosDamaNegra |= movimientosDeAlfil[casillaDamaNegra-1][key];
            if((movimientosDamaNegra & reyPropio) > 0){
                return true;
            }
        }
        U64 movimientosTorreNegra;
        U64 bitboardTorreNegra = bitboards[8];
        while(bitboardTorreNegra > 0){
            int casillaTorreNegra = operaciones_bit::LSB(bitboardTorreNegra);
        ...
        U64 movimientosAlfilNegro;
        U64 bitboardAlfilNegro = bitboards[9];
        while(bitboardAlfilNegro > 0){
            int casillaAlfilNegro = operaciones_bit::LSB(bitboardAlfilNegro);
        ...
        U64 movimientosCaballoNegro;
        U64 bitboardCaballoNegro = bitboards[10];
        while(bitboardCaballoNegro > 0){
            int casillaCaballoNegro = operaciones_bit::LSB(bitboardCaballoNegro);
        ...
        U64 movimientosPeonNegro;
        U64 bitboardPeonNegro = bitboards[11];
        while(bitboardPeonNegro > 0){
            int casillaPeonNegro = operaciones_bit::LSB(bitboardPeonNegro);
            movimientosPeonNegro = Peon::capturas(casillaPeonNegro, piezasPropias(), piezasRivales(), 1);
            if((movimientosPeonNegro & reyPropio) > 0){
                return true;
            }
        }
        U64 movimientosReyNegro;
        U64 bitboardReyNegro = bitboards[6];
        while(bitboardReyNegro > 0){
            int casillaReyNegro = operaciones_bit::LSB(bitboardReyNegro);
            movimientosReyNegro = movimientosDeRey[casillaReyNegro-1];
        ...
        U64 movimientosDamaBlanca;
        U64 bitboardDamaBlanca = bitboards[1];
        while(bitboardDamaBlanca > 0){
            int casillaDamaBlanca = operaciones_bit::LSB(bitboardDamaBlanca);
            U64 attackMaskTorre = constantes::attackMasksTorre[casillaDamaBlanca-1];
            U64 attackMaskAlfil = constantes::attackMasksAlfil[casillaDamaBlanca-1];
            U64 occupancyMask = attackMaskTorre & todas_las_piezas();
            U64 key = occupancyMask * constantes::magicsParaTorre[casillaDamaBlanca-1] >> (64-12);
            movimientosDamaBlanca = movimientosDeTorre[casillaDamaBlanca-1][key];
            occupancyMask = attackMaskAlfil & todas_las_piezas();
            key = occupancyMask * constantes::magicsParaAlfil[casillaDamaBlanca-1] >> (64-9);
            movimientosDamaBlanca |= movimientosDeAlfil[casillaDamaBlanca-1][key];
            if((movimientosDamaBlanca & reyPropio) > 0){
                return true;
            }
        }
        U64 movimientosTorreBlanca;
        U64 bitboardTorreBlanca = bitboards[2];
        while(bitboardTorreBlanca > 0){
            int casillaTorreBlanca = operaciones_bit::LSB(bitboardTorreBlanca);
        ...
        U64 movimientosAlfilBlanco;
        U64 bitboardAlfilBlanco = bitboards[3];
        while(bitboardAlfilBlanco > 0){
            int casillaAlfilBlanco = operaciones_bit::LSB(bitboardAlfilBlanco);
        ...
        U64 movimientosCaballoBlanco;
        U64 bitboardCaballoBlanco = bitboards[4];
        while(bitboardCaballoBlanco > 0){
            int casillaCaballoBlanco = operaciones_bit::LSB(bitboardCaballoBlanco);
        ...
        U64 movimientosPeonBlanco;
        U64 bitboardPeonBlanco = bitboards[5];
        while(bitboardPeonBlanco > 0){
            int casillaPeonBlanco = operaciones_bit::LSB(bitboardPeonBlanco);
            movimientosPeonBlanco = Peon::capturas(casillaPeonBlanco, piezasPropias(), piezasRivales(), 0);
            if((movimientosPeonBlanco & reyPropio) > 0){
                return true;
            }
        }
        U64 movimientosReyBlanco;
        U64 bitboardReyBlanco = bitboards[0];
        while(bitboardReyBlanco > 0){
            int casillaReyBlanco = operaciones_bit::LSB(bitboardReyBlanco);
            movimientosReyBlanco = movimientosDeRey[casillaReyBlanco-1];
...
void Tablero::actualizarOcupacion(u_short jugada) {
    ...
    if(_turno == 0){
        ...
        if (tipoDePieza == REY) {
            if(!endgame) {
                ocupacionNegras -= (constantes::ocupacionReyBlancoMedioJuego[operaciones_bit::espejarCasilla(llegada-1)]
...
bool Tablero:: desarrolloBlancasCompleto(){
    return desarrolloCaballoBlancoG() && desarrolloCaballoBlancoB() && desarrolloAlfilBlancoC() && desarrolloAlfilBlancoF();
}
